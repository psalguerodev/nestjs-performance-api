...
export class BioController {
  ...
  winInfo(@Res() response: Response) {
    this.bioService.getWinInfo().then(data => response.status(HttpStatus.OK).json(data));
  }

  @Post('/bio/api/getInfo')
  getBioInfo(@Res() response: Response) {
    this.bioService.getBioInfo().then(data => response.status(HttpStatus.OK).json(data));
  }

  @Post('/bio/api/verify')
  bioVerify(@Res() response: Response) {
   this.bioService.verifyBio().then(data => response.status(HttpStatus.OK).json(data));
  }
